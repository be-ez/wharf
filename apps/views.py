...
def wait_for_command(request, app_name, task_id, after):
    ...
    return render(
        request, 'command_wait.html', {
            'app': app_name,
            'task_id': task_id,
            'log': log,
            'state': res.state,
            'running': res.state in [state(PENDING),
                                     state(STARTED)],
...
def index(request):
    try:
        ...
        if e.__class__.__name__ in [
            ...
            return render(request, 'setup_key.html',
...
def app_config_set(app, key, value):
    return run_cmd_with_log(app, "Setting %s" % key,
                            "config:set %s %s=%s" % (app, key, value),
...
def global_config_set(request):
    ...
    if form.is_valid():
        return run_cmd_with_log(
            None, "Setting %s" % form.cleaned_data['key'],
            "config:set --global %s=%s" % (form.cleaned_data['key'],
...
def process_info(app_name):
    ...
    lines = data.split("\n")
    if lines[0].find(
            "%s process information" % app_name) == -1 and lines[0].find(
...
def add_domain(request, app_name):
    ...
    if form.is_valid():
        commands = [
            "domains:add %s %s" % (app_name, form.cleaned_data['name'])
        ]
...
def app_info(request, app_name):
    ...
    if request.method == 'POST':
        form = forms.ConfigForm(request.POST)
        if form.is_valid():
            return app_config_set(app_name, form.cleaned_data['key'],
                                  form.cleaned_data['value'])
    else:
        form = forms.ConfigForm()
    return render(
        request, 'app_info.html', {
            'postgres':
            postgres_list(app_name),
            'redis':
            redis_list(app_name),
            'letsencrypt':
            ...
            'logs':
            ansi_escape.sub("", run_cmd("logs %s --num 100" % app_name)),
            'domains':
            domains_list(app_name),
            'domain_form':
            forms.CreateDomainForm(),
            'form':
            form,
            'app':
            app_name,
            'git_url':
            config.get('GITHUB_URL', None),
            'config':
            sorted(config.items()),
            'task_logs':
            models.TaskLog.objects.filter(app=app).order_by('-when').all(),
        })
...
def deploy(request, app_name):
    ...
    models.TaskLog(
        ...
        when=datetime.now(),
        app=models.App.objects.get(name=app_name),
        description="Deploying %s" % app_name).save()
    clear_cache("config %s" % app_name)
    clear_cache("domains:report %s" % app_name)
...
def create_postgres(request, app_name):
    return run_cmd_with_log(app_name, "Add Postgres", [
        "postgres:create %s" % app_name,
        "postgres:link %s %s" % (app_name, app_name)
    ], "check_postgres")


def create_redis(request, app_name):
    return run_cmd_with_log(app_name, "Add Redis", [
        "redis:create %s" % app_name,
        "redis:link %s %s" % (app_name, app_name)
    ], "check_redis")
...
def create_app(app_name):
    ...
    return run_cmd_with_log(app_name, "Add app %s" % app_name,
                            "apps:create %s" % app_name, "check_app")
...
def setup_letsencrypt(request, app_name):
    return run_cmd_with_log(app_name, "Enable Let's Encrypt",
                            "letsencrypt %s" % app_name, "check_letsencrypt")
...
def contentful_webhook(request):
    ...
    if "HTTP_SECRET" not in request.META:
        return HttpResponseBadRequest(
            "No Secret header %s " % ' ;'.join(request.META.keys()))
    if request.META["HTTP_SECRET"] != secret:
        HttpResponseBadRequest("Bad!")
    data = json.loads(request.read())
    if not "app_name" in data:
        return HttpResponseBadRequest(
            "Can't find an entry for app_name URL %s" % "app_name")
    app_name = data["app_name"]
    cmd = "ps:rebuild %s" % app_name
    res = tasks.run_ssh_command.delay(cmd)
    app = models.App.objects.get(name=app_name)
    models.TaskLog(
        task_id=res.id,
...
def github_webhook(request):
    ...
    if data["ref"] != default_ref:
        return HttpResponse("Push to non-default branch (saw %s, expected %s)"
                            % (data["ref"], default_ref))
    clone_url = data["repository"]["clone_url"]
    apps = models.App.objects.filter(github_url=clone_url)
    if not apps.exists():
        return HttpResponseBadRequest(
            "Can't find an entry for clone URL %s" % clone_url)
    app = apps.first()
    ...
    models.TaskLog(
        ...
        when=datetime.now(),
        app=app,
        description="Deploying %s" % app.name).save()
    clear_cache("config %s" % app.name)
    return HttpResponse(
        "Running deploy. Deploy log is at %s" % request.build_absolute_uri(
            reverse('show_log', kwargs={'task_id': res.id})))
